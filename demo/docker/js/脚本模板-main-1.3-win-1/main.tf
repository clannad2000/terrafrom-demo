...
module "zygk-ui" { #【固定值】 指定module名称
  ...
  #【自定义值】指定pod的镜像名称
  containerImage          = "hub.js.sgcc.com.cn/shebeiyu/zygk-ui:1.1.3"
  #【自定义值】指定pod的命令
  containerCommand        = []
  #【自定义值】指定pod的参数
  containerArgs           = []
  ...
  hostname                = "jszygk.test.js.sgcc.com.cn"
  #【自定义值】指定ingress的配置
  annotations = {
    # --- 基本控制 ---
  ...
  paths                   = [
    { path ="/", serviceName ="zygk-ui", servicePort ="18080"}
  ]
...
########################## 3.1.1 ---------> zygk-basic-management admin pod ########################################################################
module "zygk-basic-management" { #【固定值】 指定module名称
  ...
  #【自定义值】指定pod的镜像名称
  containerImage          = "hub.js.sgcc.com.cn/shebeiyu/zygk-basic-management:1.1.5"
  #【自定义值】指定pod的命令
  containerCommand        = []
  #【自定义值】指定pod的参数
  containerArgs           = []
...
########################## 3.1.1 ---------> zygk-external-system uaa pod ########################################################################
module "zygk-external-system" { #【固定值】 指定module名称
  ...
  #【自定义值】指定pod的镜像名称
  containerImage          = "hub.js.sgcc.com.cn/shebeiyu/zygk-external-system:1.1.4"
  #【自定义值】指定pod的命令
  containerCommand        = []
  #【自定义值】指定pod的参数
  containerArgs           = []
...
########################## 3.1.1 ---------> zygk-front-end-service app-api pod ########################################################################
module "zygk-front-end-service" { #【固定值】 指定module名称
  #【固定值】指定组件入口位置
  source                  = "./modules/k8s/pod/"
  #【自定义值】指定中文名称
  zh_name                 = "外部系统对接微服务"
  ...
  #【自定义值-需确认】指定pod的svc（svcPort为svc端口，svcTargetPort为目标端口，nodePort存在则填写不存在则nodePort = "",随机生成nodePort则填0）
  svc  = [
    { name = "port",svcPort = "19520",svcTargetPort= "19520",nodePort = "" }
  ]
  #【自定义值-需确认】指定svc的ip地址
  svc_clusterip           = ""
  #【自定义值-需确认】指定pod的host配置(如果没有则填'[]')
  hostAliases             = []
  #【自定义值】指定pod的元数据名称
  metadataName            = "zygk-front-end-service"
  #【自定义值】指定pod的镜像名称
  containerImage          = "hub.js.sgcc.com.cn/shebeiyu/zygk-front-end-service:1.1.2"
  #【自定义值】指定pod的命令
  containerCommand        = []
  #【自定义值】指定pod的参数
  containerArgs           = []
